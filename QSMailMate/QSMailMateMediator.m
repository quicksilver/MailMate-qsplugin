//
//  QSMailMateMediator.m
//  QSMailMate
//
//  Created by Rob McBroom on 2013/11/12.
//

#import "QSMailMateMediator.h"

@implementation QSMailMateMediator
...
- (void)sendEmailTo:(NSArray *)addresses from:(NSString *)sender subject:(NSString *)subject body:(NSString *)body attachments:(NSArray *)pathArray sendNow:(BOOL)sendNow
{
    ...
    NSString *scriptSource = [NSString stringWithFormat:@"tell application \"MailMate\"\nopen location \"%@\" with trust\nactivate\nend tell", mailtoURL];
    NSAppleScript *script = [[NSAppleScript alloc] initWithSource:scriptSource];
    NSDictionary *error = nil;
    [script executeAndReturnError:&error];
    if (error) {
        NSLog(@"Error sending message with MailMate: %@", error[NSAppleScriptErrorMessage]);
    }
    [script release];
}
...
{
    return [QSResourceManager imageNamed:@"MailMate"];
}
